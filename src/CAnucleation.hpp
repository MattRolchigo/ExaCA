...
struct Nucleation {
    ...
    void placeNuclei(const Temperature<memory_space> &temperature, const InterfacialResponseFunction &irf, const unsigned long rng_seed, const int layernumber,
        ...
        std::vector<int> nuclei_grain_id_myrank_v(nuclei_this_layer), nuclei_location_myrank_v(nuclei_this_layer), nuclei_phase_id_myrank_v(nuclei_this_layer);
            ...
            for (int n = 0; n < nuclei_this_layer_single; n++) {
                ...
                if (((nuclei_y(n_event) > grid.y_offset) || (grid.at_south_boundary)) &&
                    ...
                    if (meltevent < number_of_solidification_events_host(nuclei_location_this_layer)) {
                        ...
                        nuclei_phase_id_myrank_v[possible_nuclei] = irf.getPreferredPhase(nuclei_undercooling_whole_domain_v[n_event]);
