...
void GhostNodes1D(int cycle, int id, int NeighborRank_North, int NeighborRank_South, int MyXSlices, int MyYSlices, int MyXOffset,
    ...
    // Send data to each other rank (MPI_Isend)
    MPI_Isend(BufferSouthSend.data(), 4 * BufSizeX * BufSizeZ, MPI_FLOAT, NeighborRank_South, 0, MPI_COMM_WORLD,
              &SendRequests[0]);
    ...
    // Receive buffers for all neighbors (MPI_Irecv)
    MPI_Irecv(BufferSouthRecv.data(), 4 * BufSizeX * BufSizeZ, MPI_FLOAT, NeighborRank_South, 0, MPI_COMM_WORLD,
              &RecvRequests[0]);
    ...
    // Wait on send requests
    MPI_Waitall(4, SendRequests.data(), MPI_STATUSES_IGNORE);

    // Unpack both buffers at the same time
    Kokkos::parallel_for(
        "BufferUnpack", BufSizeX * BufSizeZ, KOKKOS_LAMBDA(const int &BufPosition) {
            for (int unpack_index = 0; unpack_index < 2; unpack_index++) {
                ...
                if ((unpack_index == 0) && (NeighborRank_South != MPI_PROC_NULL)) {
                    ...
                    CellLocation = RankZ * MyXSlices * MyYSlices + MyYSlices * RankX + RankY;
                    int GlobalCellLocation = CellLocation + ZBound_Low * MyXSlices * MyYSlices;
                    if ((BufferSouthRecv(BufPosition, 3) > 0) && (CellType(GlobalCellLocation) == Liquid)) {
                        Place = true;
                        NewGrainID = BufferSouthRecv_I(BufPosition);
                        DOCenterX = BufferSouthRecv(BufPosition, 0);
                        DOCenterY = BufferSouthRecv(BufPosition, 1);
                        DOCenterZ = BufferSouthRecv(BufPosition, 2);
                        NewDiagonalLength = BufferSouthRecv(BufPosition, 3);
                    }
                }
                else if ((unpack_index == 1) && (NeighborRank_North != MPI_PROC_NULL)) {
                    // Data received from North
                    ...
                    CellLocation = RankZ * MyXSlices * MyYSlices + MyYSlices * RankX + RankY;
                    int GlobalCellLocation = CellLocation + ZBound_Low * MyXSlices * MyYSlices;
                    if ((BufferNorthRecv(BufPosition, 3) > 0) && (CellType(GlobalCellLocation) == Liquid)) {
                        Place = true;
                        NewGrainID = BufferNorthRecv_I(BufPosition);
                        DOCenterX = BufferNorthRecv(BufPosition, 0);
                        DOCenterY = BufferNorthRecv(BufPosition, 1);
                        DOCenterZ = BufferNorthRecv(BufPosition, 2);
                        NewDiagonalLength = BufferNorthRecv(BufPosition, 3);
                    }
                }
                if (Place) {
                    int GlobalZ = RankZ + ZBound_Low;
                    ...
                    DOCenter((long int)(3) * CellLocation + 1) = DOCenterY;
                    DOCenter((long int)(3) * CellLocation + 2) = DOCenterZ;
                    int MyOrientation = getGrainOrientation(GrainID(GlobalCellLocation), NGrainOrientations);
                    ...
                    // liquid cell
                    calcCritDiagonalLength(CellLocation, xp, yp, zp, DOCenterX, DOCenterY, DOCenterZ, NeighborX,
                                           NeighborY, NeighborZ, MyOrientation, GrainUnitVector, CritDiagonalLength);
                    CellType(GlobalCellLocation) = Active;
                }
            }
        });
