...
void RunProgram_Reduced(int id, int np, std::string InputFile) {
    ...
    int PrintDebug, MovieFrameInc;
    bool PrintMisorientation, PrintFullOutput, RemeltingYN, UseSubstrateFile, PrintMovieFrames, PrintIdleMovieFrames;
    ...
    InputReadFromFile(id, InputFile, SimulationType, DecompositionStrategy, AConst, BConst, CConst, DConst,
                      ...
                      NSpotsY, SpotOffset, SpotRadius, PrintMovieFrames, MovieFrameInc, PrintIdleMovieFrames);
    ...
    for (int layernumber = 0; layernumber < NumberOfLayers; layernumber++) {
        ...
        do {
            ...
            if ((PrintMovieFrames) && (cycle % MovieFrameInc == 0)) {
            ...
            if (cycle % 1000 == 0) {
                IntermediateOutputAndCheck(id, np, cycle, MyXSlices, MyYSlices, LocalDomainSize, LocalActiveDomainSize,
                                           nx, ny, nz, nzActive, deltax, XMin, YMin, ZMin, DecompositionStrategy,
                                           ProcessorsInXDirection, ProcessorsInYDirection, nn, XSwitch, CellType_G,
                                           CellType_H, CritTimeStep_G, CritTimeStep_H, GrainID_G, GrainID_H,
                                           SimulationType, FinishTimeStep, layernumber, NumberOfLayers, ZBound_Low,
                                           NGrainOrientations, Melted, LayerID_G, LayerID_H, GrainOrientation_H,
                                           GrainUnitVector_H, UndercoolingChange_H, UndercoolingCurrent_H, PathToOutput,
                                           OutputFile, PrintIdleMovieFrames, MovieFrameInc, IntermediateFileCounter);
            }
