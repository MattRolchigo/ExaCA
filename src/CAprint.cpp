...
void PrintExaCAData(int id, int layernumber, int np, int nx, int ny, int nz, int MyXSlices, int MyYSlices,
                    ...
                    int PrintDebug, bool PrintMisorientation, bool PrintFullOutput, bool PrintMovieFrame,
    ...
    if (id == 0) {
        ...
        if ((PrintMovieFrame) || (PrintMisorientation) || (PrintDebug == 2) || (PrintFullOutput)) {
        ...
        if ((PrintDebug > 0) || (PrintMovieFrame)) {
        ...
        if (PrintMovieFrame)
            PrintMovie(IntermediateFileCounter, layernumber, BaseFileName, PathToOutput, ZBound_Low, nzActive, nx, ny,
                       GrainID_WholeDomain, CellType_WholeDomain, GrainOrientation, GrainUnitVector, NGrainOrientations,
...
// Prints grain misorientation and cell type as a movie frame
...
void PrintMovie(int IntermediateFileCounter, int layernumber, std::string BaseFileName, std::string PathToOutput,
                int ZBound_Low, int nzActive, int nx, int ny, ViewI3D_H GrainID_WholeDomain,
                ViewI3D_H CellType_WholeDomain, ViewI_H GrainOrientation, ViewF_H GrainUnitVector,
