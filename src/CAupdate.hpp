    ...
    Kokkos::parallel_for(
        "InitSV", grid.domain_size, KOKKOS_LAMBDA(const int &index) {
            if (celldata.cell_type(index) == Active) {
                ...
                // Add to steering vector for first iteration
                            ...
                            for (int l = 0; l < 26; l++) {
                                ...
                                if ((celldata.cell_type(neighbor_index) == Solid) || (coord_z == 0)) {
                                    ...
                                    interface.steering_vector_new(
                                        Kokkos::atomic_fetch_add(&interface.num_steer_new(0), 1)) = index;
...
void cellCapture(const int cycle, const int np, const Grid &grid, const InterfacialResponseFunction &irf,
    ...
    // Reset size of new steering vector to 0 (cells will be added to it on this time step)
                ...
                for (int l = 0; l < 26; l++) {
                    ...
                    if ((diagonal_length_cell >= interface.crit_diagonal_length(26 * index + l)) &&
                        ...
                        if (old_cell_type_value == Liquid) {
                            ...
                            const float diag_1y = orientation.grain_unit_vector(9 * my_orientation + 1, my_phase_id) *
                                                  (2 * angle_1_pos - 1);
                            const float diag_1z = orientation.grain_unit_vector(9 * my_orientation + 2, my_phase_id) *
                                                  (2 * angle_1_pos - 1);

                            const float diag_2x = orientation.grain_unit_vector(9 * my_orientation + 3, my_phase_id) *
                                                  (2 * angle_2_pos - 1);
                            const float diag_2y = orientation.grain_unit_vector(9 * my_orientation + 4, my_phase_id) *
                                                  (2 * angle_2_pos - 1);
                            const float diag_2z = orientation.grain_unit_vector(9 * my_orientation + 5, my_phase_id) *
                                                  (2 * angle_2_pos - 1);

                            const float diag_3x = orientation.grain_unit_vector(9 * my_orientation + 6, my_phase_id) *
                                                  (2 * angle_3_pos - 1);
                            const float diag_3y = orientation.grain_unit_vector(9 * my_orientation + 7, my_phase_id) *
                                                  (2 * angle_3_pos - 1);
                            const float diag_3z = orientation.grain_unit_vector(9 * my_orientation + 8, my_phase_id) *
                                                  (2 * angle_3_pos - 1);
        ...
        "FillSendBuffersOverflow", grid.nx, KOKKOS_LAMBDA(const int &coord_x) {
            for (int coord_z = 0; coord_z < grid.nz_layer; coord_z++) {
                ...
                if (celldata.cell_type(index_south_buffer) == ActiveFailedBufferLoad) {
                    ...
                    bool data_fits_in_buffer = interface.loadGhostNodes(
                        ghost_grain_id, ghost_octahedron_center_x, ghost_octahedron_center_y, ghost_octahedron_center_z,
                        ghost_diagonal_length, ghost_phase_id, grid.ny_local, coord_x, 2, coord_z,
                    ...
                    bool data_fits_in_buffer =
                        interface.loadGhostNodes(-1, -1.0, -1.0, -1.0, 0.0, 1, grid.ny_local, coord_x, 2, coord_z,
                                                 grid.at_north_boundary, grid.at_south_boundary, n_grain_orientations);
                ...
                if (celldata.cell_type(index_north_buffer) == ActiveFailedBufferLoad) {
                    ...
                    bool data_fits_in_buffer = interface.loadGhostNodes(
                        ghost_grain_id, ghost_octahedron_center_x, ghost_octahedron_center_y, ghost_octahedron_center_z,
                        ghost_diagonal_length, ghost_phase_id, grid.ny_local, coord_x, grid.ny_local - 3, coord_z,
                        grid.at_north_boundary, grid.at_south_boundary, n_grain_orientations);
                    ...
                    // diagonal length
                    bool data_fits_in_buffer = interface.loadGhostNodes(
                        -1, -1.0, -1.0, -1.0, 0.0, 1, grid.ny_local, coord_x, grid.ny_local - 3, coord_z,
                        grid.at_north_boundary, grid.at_south_boundary, n_grain_orientations);
