...
template <typename MemorySpace>
void cellCaptureSV(const int cycle, const int np, const Grid &grid, const InterfacialResponseFunction &irf,
    ...
    auto grain_id = celldata.getGrainIDSubview(grid);
    const int num_steer_host_old = interface.num_steer_host(0);
    // Loop over list of active and soon-to-be active cells, adding cells associated with potential capture events to a
    // new steering vector
    Kokkos::parallel_for(
...
// Decentered octahedron algorithm for the capture of new interface cells by grains
template <typename MemorySpace>
void cellCapture(const int cycle, const int np, const Grid &grid, CellData<MemorySpace> &celldata,
                 Temperature<MemorySpace> &temperature, Interface<MemorySpace> &interface,
                 Orientation<MemorySpace> &orientation) {

    // Get grain_id subview for this layer
    auto grain_id = celldata.getGrainIDSubview(grid);
    // Loop over list of active cells marked as associated with a cell capture event on this time step
    Kokkos::parallel_for(
        "CellCapture", interface.num_steer_cc_host(0), KOKKOS_LAMBDA(const int &num) {
            // Reset CC steering vector size on device to 0, to be rebuilt next time step
            interface.num_steer_cc(0) = 0;
            // Get the 1D index of cell from the steering vector
            const int index = interface.cc_steering_vector(num);
            const int capt_dir = interface.cc_steering_vector_idx(num);
            // Using the 1D index, get the x, y, z coordinates of the cell on this MPI rank
            const int coord_x = grid.getCoordX(index);
            const int coord_y = grid.getCoordY(index);
            const int coord_z = grid.getCoordZ(index);
            // Local coordinates of captured cell center
            const int neighbor_coord_x = coord_x + interface.neighbor_x[capt_dir];
            const int neighbor_coord_y = coord_y + interface.neighbor_y[capt_dir];
            const int neighbor_coord_z = coord_z + interface.neighbor_z[capt_dir];
            //                // Check if neighbor is in bounds
            ...
            const int angle_1_pos = ((orientation.grain_unit_vector(9 * my_orientation) * x0 +
                                      orientation.grain_unit_vector(9 * my_orientation + 1) * y0 +
                                      orientation.grain_unit_vector(9 * my_orientation + 2) * z0) > 0);
            const int angle_2_pos = ((orientation.grain_unit_vector(9 * my_orientation + 3) * x0 +
                                      orientation.grain_unit_vector(9 * my_orientation + 4) * y0 +
                                      orientation.grain_unit_vector(9 * my_orientation + 5) * z0) > 0);
            const int angle_3_pos = ((orientation.grain_unit_vector(9 * my_orientation + 6) * x0 +
                                      orientation.grain_unit_vector(9 * my_orientation + 7) * y0 +
                                      orientation.grain_unit_vector(9 * my_orientation + 8) * z0) > 0);
            const float diag_1x = orientation.grain_unit_vector(9 * my_orientation) * (2 * angle_1_pos - 1);
            const float diag_1y = orientation.grain_unit_vector(9 * my_orientation + 1) * (2 * angle_1_pos - 1);
            const float diag_1z = orientation.grain_unit_vector(9 * my_orientation + 2) * (2 * angle_1_pos - 1);

            const float diag_2x = orientation.grain_unit_vector(9 * my_orientation + 3) * (2 * angle_2_pos - 1);
            const float diag_2y = orientation.grain_unit_vector(9 * my_orientation + 4) * (2 * angle_2_pos - 1);
            const float diag_2z = orientation.grain_unit_vector(9 * my_orientation + 5) * (2 * angle_2_pos - 1);

            const float diag_3x = orientation.grain_unit_vector(9 * my_orientation + 6) * (2 * angle_3_pos - 1);
            const float diag_3y = orientation.grain_unit_vector(9 * my_orientation + 7) * (2 * angle_3_pos - 1);
            const float diag_3z = orientation.grain_unit_vector(9 * my_orientation + 8) * (2 * angle_3_pos - 1);
            ...
            const float crit_diagonal_length_capture = interface.crit_diagonal_length(26 * index + capt_dir);
            ...
            if (np > 1) {
                ...
                bool data_fits_in_buffer = interface.loadGhostNodes(
                    ...
                    ghost_diagonal_length, grid.ny_local, neighbor_coord_x, neighbor_coord_y, neighbor_coord_z,
                    grid.at_north_boundary, grid.at_south_boundary, orientation.n_grain_orientations);
                if (!(data_fits_in_buffer)) {
                    // This cell's data did not fit in the buffer with current size buf_size -
                    // mark with temporary type
...
template <typename MemorySpace>
void rebuildSteeringVector(Interface<MemorySpace> &interface, CellData<MemorySpace> &celldata) {
    const int steer_size_after_capture = interface.num_steer_host(0);
    int new_steer_size;
    // Parallel scan to reconstruct steering vector, removing cells that have transformed into a type without any
    // associated calculations or checks
    Kokkos::parallel_scan(
        "create_new_sv", steer_size_after_capture,
        KOKKOS_LAMBDA(const int num, int &chunk_start, bool is_final) {
            ...
            // else {
            //    if (index == 10690125)
            //       printf("REMOVING CELL %d FROM THE SV, TYPE WAS %d\n",num+chunk_start,cell_type_local);
            // }
        },
        new_steer_size);
    Kokkos::fence();
    // Overwrite steering_vector with steering_vector_new
    Kokkos::deep_copy(interface.steering_vector, interface.steering_vector_new);
    // New steering vector size on host and copied to device
    interface.num_steer_host(0) = new_steer_size;
    Kokkos::deep_copy(interface.num_steer, interface.num_steer_host);
}
