...
void cellCapture(const int cycle, const int np, const Grid &grid, CellData<MemorySpace> &celldata,
    ...
    // cell types
    Kokkos::parallel_for(
        "CellCapture", interface.num_steer_cc(0), KOKKOS_LAMBDA(const int &num) {
            ...
            const int index = interface.steering_vector_cc(num, 0);
            const int l = interface.steering_vector_cc(num, 1);
            ...
            int old_cell_type_value =
                Kokkos::atomic_compare_exchange(&celldata.cell_type(neighbor_index), Liquid, TemporaryUpdate);
            ...
            if (old_cell_type_value == Liquid) {
                ...
                const int angle_1_pos = ((orientation.grain_unit_vector(9 * my_orientation, my_phase_id) * x0 +
                ...
                const int angle_2_pos = ((orientation.grain_unit_vector(9 * my_orientation + 3, my_phase_id) * x0 +
                ...
                const int angle_3_pos = ((orientation.grain_unit_vector(9 * my_orientation + 6, my_phase_id) * x0 +
                ...
                if (np > 1) {
                    ...
                    bool data_fits_in_buffer = interface.loadGhostNodes(
                        ...
                        ghost_diagonal_length, ghost_phase_id, grid.ny_local, neighbor_coord_x, neighbor_coord_y,
                        neighbor_coord_z, grid.at_north_boundary, grid.at_south_boundary,
...
// Decentered octahedron algorithm for the capture of new interface cells by grains
template <typename MemorySpace>
void createCellCaptureSteeringVector(const int cycle, const int np, const Grid &grid,
                                     const InterfacialResponseFunction &irf, CellData<MemorySpace> &celldata,
                                     Temperature<MemorySpace> &temperature, Interface<MemorySpace> &interface,
                                     Orientation<MemorySpace> &orientation) {
                    ...
                    if (neighbor_index != -1) {
                        ...
                        if (neighbor_cell_type == Liquid) {
                            ...
                            if (diagonal_length_cell >= interface.crit_diagonal_length(26 * index + l)) {
                                const int cc_steer_pos = Kokkos::atomic_fetch_add(&interface.num_steer_cc(0), 1);
                                interface.steering_vector_cc(cc_steer_pos, 0) = index;
                                interface.steering_vector_cc(cc_steer_pos, 1) = l;
                            }
                        }
                    }
                }
